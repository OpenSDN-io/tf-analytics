...
void SyslogParser::Parse (SyslogQueueEntry *sqe) {
    work_queue_.Enqueue (sqe);
}

void SyslogParser::Shutdown ()
{
...
bool SyslogParser::parse_syslog (Iterator start, Iterator end, syslog_m_t &v)
{
    using qi::int_;
    using qi::_1;
    using ascii::space;
    ...
    bool r = qi::phrase_parse(start, end,
        ...
        phx::construct<std::pair<std::string, Holder> >("hostname",
                  phx::construct<Holder>("hostname", _1)))]
                               //version
               |int_   [insert(phx::ref(v),
        phx::construct<std::pair<std::string, Holder> >("version",
                  phx::construct<Holder>("version", _1)))]
                               //year
               >>int_       [insert(phx::ref(v),
        ...
        phx::construct<std::pair<std::string, Holder> >("month",
               ...
               >> int_         [insert(phx::ref(v),
        phx::construct<std::pair<std::string, Holder> >("day",
                  phx::construct<Holder>("day", _1)))] >> lit("T")
                               //hour
               >> int_         [insert(phx::ref(v),
        phx::construct<std::pair<std::string, Holder> >("hour",
                  phx::construct<Holder>("hour", _1)))] >> lit(":")
        ...
        phx::construct<std::pair<std::string, Holder> >("min",
               ...
               >> int_         [insert(phx::ref(v),
        phx::construct<std::pair<std::string, Holder> >("sec",
                  phx::construct<Holder>("sec", _1)))] >> lit(".")
                               //msec
               >> int_         [insert(phx::ref(v),
        phx::construct<std::pair<std::string, Holder> >("msec",
                  phx::construct<Holder>("msec", _1)))]
               >>( lit("Z")
                               // tz
               |word            [insert(phx::ref(v),
        phx::construct<std::pair<std::string, Holder> >("tz",
                  phx::construct<Holder>("tz", _1)))]
                  )
                               //hostname
               >>  word        [insert(phx::ref(v),
        phx::construct<std::pair<std::string, Holder> >("hostname",
...
std::string SyslogParser::GetMapVals (syslog_m_t v, std::string key, std::string def)
{
...
int64_t SyslogParser::GetMapVal (syslog_m_t v, std::string key, int def)
{
    std::map<std::string, Holder>::iterator i = v.find (key);
    if (i == v.end())
        return def;
...
void SyslogParser::GetFacilitySeverity (syslog_m_t v, int& facility, int& severity)
{
    int fs = GetMapVal (v, "facsev", 0);
    severity = fs & 0x7;
    facility = fs >> 3;
}

void SyslogParser::GetTimestamp (syslog_m_t v, time_t& timestamp)
{
    bt::ptime lt(bt::microsec_clock::local_time());
    bt::ptime ut(bt::microsec_clock::universal_time());
    tm pt_tm = bt::to_tm(lt);
    bt::ptime p(boost::gregorian::date (GetMapVal (v, "year", pt_tm.tm_year+1900),
...
void SyslogParser::PostParsing (syslog_m_t &v) {
  time_t timestamp;
...
std::string SyslogParser::GetSyslogFacilityName (uint64_t f)
{
...
std::string SyslogParser::EscapeXmlTags (std::string text)
{
    std::ostringstream s;
#ifdef SYSLOG_DEBUG
...
std::string SyslogParser::GetMsgBody (syslog_m_t v) {
    return EscapeXmlTags (GetMapVals (v, "body", ""));
}

std::string SyslogParser::GetModule(syslog_m_t v) {
    return GetMapVals(v, "prog", "UNKNOWN");
}

std::string SyslogParser::GetFacility(syslog_m_t v) {
    return GetSyslogFacilityName(GetMapVal(v, "facility", 0));
}

int SyslogParser::GetPID(syslog_m_t v) {
